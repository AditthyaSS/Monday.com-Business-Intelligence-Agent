...
**D5. Gemini free tier (Flash-class model) via `google-genai`, behind `LLMProvider`.**
Why: only free option available. Consequences: max 2 LLM calls per question, coarse tools with enum params,
retry/backoff on 429, Flash to Flash-Lite fallback, response cache, per-IP throttle, sample-question chips.
Model name and limits: to confirm in AI Studio (sources conflict).
